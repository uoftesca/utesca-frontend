...
type FileUploadState = {
  status:
    "idle" | "compressing" | "uploading" | "uploaded" | "error" | "deleting";
  file?: File;
  url?: string;
  fileId?: string;
  error?: string;
...
export function RegistrationForm({ event, slug }: RegistrationFormProps) {
  ...
  if (!schema) {
    return (
      <div className="w-full max-w-3xl space-y-4">
        <Button variant="ghost" size="lg" asChild className="-ml-4">
  ...
  const eventDateDisplay = formatEventDate(event.dateTime);
  const eventTimeDisplay = formatEventTime(event.dateTime);
  const deadlineDisplay = formatDate(event.registrationDeadline);

  if (isClosed) {
    return (
      <div className="w-full max-w-5xl space-y-6">
        <Button variant="ghost" size="lg" asChild className="-ml-4 w-fit">
          <Link href="/events" className="inline-flex items-center gap-2">
            <ArrowLeft className="h-4 w-4" />
            Back to Events
          </Link>
  ...
  const handleFileUpload = async (fieldId: string, file: File | null) => {
    ...
    if (file.type.startsWith("image/")) {
      setFileUploads((prev) => ({
        ...prev,
        [fieldId]: { status: "compressing", file },
      }));
      try {
    ...
    if (fileToUpload.size > maxSize) {
      ...
      setFileUploads((prev) => ({
        ...
        [fieldId]: {
          status: "error",
          file,
          error: `File must be under ${maxMB}MB even after compression.`,
        },
      }));
      return;
    }
    ...
    return (
      <div className="flex flex-col gap-2">
        <label
          ...
          className={`flex w-full cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed px-4 py-6 text-center text-base transition ${
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="font-medium text-base">{uploadLabel}</span>
        ...
        />
        <span className="text-sm text-muted-foreground">
          {formatAllowedTypes(field.validation?.allowedTypes)}, max{" "}
          {Math.round((field.validation?.maxSize ?? 8_388_608) / (1024 * 1024))}
          MB. Files are stored after successful upload.
        </span>
    ...
    <div className="w-full max-w-5xl space-y-8">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2 text-base text-foreground">
          ...
          {deadlineDisplay && (
            <div className="flex items-center gap-2 text-muted-foreground">
              <span>Registration closes on {deadlineDisplay}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <form className="space-y-8" onSubmit={handleSubmit}>
        ...
        {successMessage ? (
          <div className="space-y-4 rounded-xl border border-emerald-300 bg-emerald-50 p-5 text-base text-emerald-900">
          ...
          <>
            {schema.autoAccept && (
              <Alert className="border-primary/30 bg-primary/5 text-primary">
                ...
                <AlertTitle>
                  You will automatically be accepted to this event.
                </AlertTitle>
                <AlertDescription>
